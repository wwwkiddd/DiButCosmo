...
async def back_to_start_ai(message: types.Message, state: FSMContext):
...
async def work_with_text(message: types.Message, state: FSMContext):
    ...
    await message.answer('В этом разделе представлены 2 генеративные модели:\n\n'
                         ...
                         '2) Продвинутая модель! Лучшая нейросеть для <i>учебы и работы</i>, в ней сделан акцент на <b>веб-поиск и проверку фактов</b>\n'
...
async def enter_to_gemini(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer('Выбранная модель: <b>Лёгкая 🌠</b>\nНапишите запрос...',
                                  reply_markup=text_kbd)


@ai_func.callback_query(F.data == 'perplexity')
async def enter_to_perplexity(callback: types.CallbackQuery, state: FSMContext):
    await callback.answer()
    await callback.message.answer('Выбранная модель: <b>Продвинутая 👨‍💻</b>\nНапишите запрос...',
                                  reply_markup=text_kbd)
...
async def work_with_image(message: types.Message, state: FSMContext):
    ...
    await message.answer('✨ <b>Создавайте и редактируйте фото прямо в чате!</b> ✨\n\n'
                                                    'Чтобы начать, отправьте изображение, которое вы хотите изменить, и напишите запрос или просто напишите в чат, что нужно создать\n\n'
...
Стоимость запроса: 10 токенов''', reply_markup=text_kbd)
